...
module.exports = class swing extends attObj{
	constructor(player,num,angle,buttons,pullback,hangTime,swingAngle,distance,size,damage,strength,launchAngle,recover,grounded,stuck=grounded,stun=recover){
        ...
        this.swingAngle = swingAngle*Math.PI/180;
    }  
    /*
    activate(){
    ...
    */
    getCurrBox(){
        ...
        if (this.frame<this.pullback){
            let swingRat = this.frame/(this.pullback+this.hangTime);
            ...
            for (let i = 0;i<numBalls;i++){
                ...
                let tx = (this.player.position[0]+this.player.velocity[0]) + temX*Math.sin(this.angle-(Math.PI) - (this.swingAngle*swingRat));
                let ty = (this.player.position[1]+this.player.velocity[1]) + temY*Math.cos(this.angle-(Math.PI) - (this.swingAngle*swingRat));
            ...
            let numBalls = this.distance/this.size;
            for (let i = 0;i<numBalls;i++){
                let temX = ((i*this.size*this.actFace));
                let temY = ((i*this.size));
